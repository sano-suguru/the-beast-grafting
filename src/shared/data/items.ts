import type { ItemData, ItemId } from "../types";
import { EQUIPS } from "./equips";

export const ITEMS: Record<ItemId, ItemData> = {
  preservative: {
    id: "preservative",
    name: "濁った防腐液",
    cost: 3,
    atk: 1,
    hp: 1,
    equip: null,
    skillText: "対象に+1/+1",
    lore: "腐敗の進行を遅らせ、わずかに筋繊維を硬化させる安価な薬品。気休め程度にはなる。",
  },
  iron_plate: {
    id: "iron_plate",
    name: EQUIPS.iron.name,
    cost: 3,
    atk: 0,
    hp: 0,
    equip: "iron",
    skillText: EQUIPS.iron.desc,
    lore: "処刑された騎士の鎧の破片。素体の皮膚に直接太いワイヤーで縫い付けることで、致命傷を防ぐ。",
  },
  bile: {
    id: "bile",
    name: EQUIPS.berserk.name,
    cost: 3,
    atk: 0,
    hp: 0,
    equip: "berserk",
    skillText: EQUIPS.berserk.desc,
    lore: "異端審問で拷問死した男の胆嚢から抽出した黒い液体。注射された素体は、痛覚を失い狂乱状態に陥る。",
  },
  maggot: {
    id: "maggot",
    name: EQUIPS.maggot_nest.name,
    cost: 3,
    atk: 0,
    hp: 0,
    equip: "maggot_nest",
    skillText: EQUIPS.maggot_nest.desc,
    lore: "腐った肉に群がる白い蛆虫の塊。素体の傷口に押し込めば、宿主の死後も蠢き続け、新たな肉体として這い出してくる。",
  },
  numbness: {
    id: "numbness",
    name: EQUIPS.numbness.name,
    cost: 3,
    atk: 0,
    hp: 0,
    equip: "numbness",
    skillText: EQUIPS.numbness.desc,
    lore: "脊髄に直接注入する乳白色の毒液。痛みを感じなくなった素体は、致命傷を負っても平然と動き続ける。",
  },
  acid_blood: {
    id: "acid_blood",
    name: EQUIPS.acid.name,
    cost: 3,
    atk: 0,
    hp: 0,
    equip: "acid",
    skillText: EQUIPS.acid.desc,
    lore: "深淵の蟲から採取した腐食性の体液。素体の血管に流し込めば、斬りつけた傷口から飛沫が迸り、周囲の肉をも溶かす。",
  },
  corpse_wax: {
    id: "corpse_wax",
    name: EQUIPS.corpse_wax.name,
    cost: 3,
    atk: 0,
    hp: 0,
    equip: "corpse_wax",
    skillText: EQUIPS.corpse_wax.desc,
    lore: "屍蝋で固められた人皮の盾。死者の脂肪が奇跡的な硬度を持ち、致命の一撃を一度だけ防ぐ。",
  },
  death_curse: {
    id: "death_curse",
    name: EQUIPS.death_curse.name,
    cost: 3,
    atk: 0,
    hp: 0,
    equip: "death_curse",
    skillText: EQUIPS.death_curse.desc,
    lore: "死刑囚の皮膚に刻まれていた呪いの紋様。これを縫い付けた素体は、死してなお怨念で肉体を繋ぎ止め、一度だけ立ち上がる。",
  },
  pure_blood: {
    id: "pure_blood",
    name: "純血",
    cost: 0,
    atk: 1,
    hp: 2,
    equip: null,
    skillText: "対象に+1/+2",
    lore: "聖杯から生成された純粋な生命力。一滴垂らすだけで、朽ちかけた肉が脈打ち始める。対価は、何も要らない。",
  },
};
