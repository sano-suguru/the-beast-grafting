...
function getCardClass(border: string, cantAfford: boolean, isEquip: boolean): string {
  ...
  return `w-12 md:w-16 shrink-0 aspect-[3/4] ${bg} border ${border} rounded-md relative flex flex-col cursor-pointer transition-all ${hover} select-none`;
}
    ...
    >
      <div className="flex flex-1 flex-col p-1">
        ...
        <div className="pointer-events-none flex flex-1 flex-col items-center justify-center gap-0.5">
          <span className="text-[6px] tracking-wider text-zinc-500">
            {isEquip ? "装備" : "消費"}
          </span>
          {isPureBlood ? (
