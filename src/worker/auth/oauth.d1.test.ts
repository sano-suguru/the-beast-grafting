...
describe("fetchUserInfo", () => {
  it("extracts Discord providerId", async () => {
    mockFetch(200, { id: "123" });
    const result = await fetchUserInfo("discord", "token");
    expect(result.isOk()).toBe(true);
    if (result.isOk()) expect(result.value.providerId).toBe("123");
  });

  it("extracts Google providerId from sub field", async () => {
    mockFetch(200, { sub: "g-789" });
    const result = await fetchUserInfo("google", "token");
    expect(result.isOk()).toBe(true);
    if (result.isOk()) expect(result.value.providerId).toBe("g-789");
  });
...
describe("findOrCreateByProvider", () => {
  ...
  it("preserves display name when linking", async () => {
    const now = new Date();
  ...
  it("preserves guest name when linking via OAuth", async () => {
    const now = new Date();
    ...
    expect(rows[0]?.displayName).toBe("名もなき術師#1234");
  });

  it("assigns guest name to new player created via OAuth", async () => {
    const result = await findOrCreateByProvider(db, "discord", userInfo, null);
    expect(result.isOk()).toBe(true);
    if (!result.isOk()) return;
    ...
    const rows = await db
      .select({ displayName: players.displayName })
      .from(players)
      .where(eq(players.id, result.value.playerId));
    expect(rows[0]?.displayName).toMatch(/^名もなき術師#/);
  });
});
